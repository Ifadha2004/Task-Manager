...
export default function MyTasksPage() {
  ...
  const newTaskLink = `/tasks/new?assignee=${encodeURIComponent(uid)}`;
          ...
          {(["all","overdue","week"] as Quick[]).map(k=>(
            <button
              ...
              className={`px-3 py-1 rounded-md border text-sm ${
                quick===k ? "border-neon text-neon bg-neon/10" : "border-border text-zinc-300"
              }`}
            >
      ...
      {(!loading && view.length===0) ? (
        <div className="rounded-xl border border-border bg-panel p-6 text-zinc-300">
          ...
          <div className="text-sm">
            Create one now →{" "}
            <Link to={newTaskLink} className="text-neon underline">+ New Task</Link>
          </div>
        </div>
      ) : (
